 ...
 * 而且受关键词路由约束（没命中任何分类的关键词就跳过）；模型追查细节、换个角度、
...
public class KnowledgeSearchTool {
    ...
    @Tool(name = "knowledgeSearch", description = """
            检索健康知识库（饮食、运动、药物，以及后台上传的其他分类资料），返回命中的资料原文片段。
            什么时候用：需要具体的数字或事实（食物热量、运动消耗、食谱、药物说明、护肤之类专题资料）；
        ...
        try {
            ...
            if (!result.decision().retrieve()) {
                ...
                return "这个问题按关键词判定与健康知识库无关，没有发起检索。"
                        + "如果确实需要事实性资料，请把问题问得更具体"
                        + "（带上具体的食物、运动、药物，或者资料所属的那个分类名）。";
            }
