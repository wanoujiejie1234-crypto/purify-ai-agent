...
public class RagProperties {
    ...
    public static class Router {
         ...
         *
         * <p><b>这里只是「内置分类表」，不是「全部分类」。</b>知识库管理页可以选
         * 「其他…」自己填一个类型名，那些分类会随文档存进切片元数据，
         * 并由 {@code KnowledgeCategories} 发现出来后一起参与路由、检索过滤和下拉框渲染。
         * 需要「全部分类」的地方请注入那个接口，<b>不要直接读这个列表</b>——
         * 读了它就会漏掉用户自建的分类，而漏掉的表现是「文档传进去了、就是检索不到」。
         *
         * <p>自建分类的关键词只有类型名自己，所以想让一类提问稳定命中某个自建分类，
         * 要么把类型名起得贴合提问里的说法，要么在这里补一条正儿八经的关键词表。
         */
        private List<Category> categories = new ArrayList<>();

        /**
         * 是否把「库里已经用过的分类」也纳入路由。<b>默认开</b>：关掉它就等价于
         * 这个功能没加进来——管理页上仍然能选「其他…」、文档也照样入库，
         * 但那些分类不会被路由认识，于是查不到（理由见 {@code PgVectorKnowledgeCategories}）。
         *
         * <p>留这个开关是给「库里出现了脏分类值、把路由搞乱」这种场面兜底：
         * 关掉它，路由立刻退回只用内置那几项，不用改代码也不用改库。
         */
        private boolean discoverFromStore = true;

        /**
         * 上面那次发现的缓存时长（秒）。0 表示每次都重新发现。
         *
         * <p>这个查询在每一轮对话的路由里都要跑，而它是一列上的 DISTINCT
         * （通常没有索引），所以默认缓存半分钟。入库和删除之后会显式失效，
         * 所以「刚传上去的类型立刻可用」不靠这个时长保证——它只决定
         * 「别处改了库、本应用多久看见」。
         */
        private int discoverCacheSeconds = 30;
