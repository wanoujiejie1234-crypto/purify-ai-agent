 ...
 * 知识库分类（内置的那几个，以及用户在界面上自建的类型名，都要原样发给后端做等值过滤；
 * 连列表本身都由 `GET /api/knowledge/categories` 直出，这边只翻「分类」这个标签，
 * 以及界面上那个哨兵项「其他…」，见 `views/KnowledgeView.vue` 的 OTHER）、
...
export default {
  ...
  knowledge: {
    ...
    upload: {
      ...
      category: '分类',
      // 分类列表还没拉回来时的占位项（拉失败时也停在这一项上）
      categoryPlaceholder: '请选择…',
      // 分类列表由后端直出（内置的 + 库里已经用过的），前端不写死；
      // 只有这一项是界面自己加的哨兵，选中它才让用户手填类型名。
      // 逗号用中文的，和上面的分隔符口径一致
      categoryOther: '其他…',
      categoryCustomLabel: '类型名',
      categoryCustomPlaceholder: '例如：护肤',
      // 这段话要说清两件事：它会成为一个独立分类；路由是纯字符串匹配，
      // 提问里得出现这个名字才会按它过滤。不说的话，用户传完发现「检索不到」会来问。
      // {name} 是用户当前填的那个名字（没填时是「（还没填）」），
      // 摆在句子里而不是另起一行 —— 用户改一个字，这句话就跟着变
      categoryCustomHint: '会作为一个新分类「{name}」入库。检索按关键词匹配，提问里出现这个类型名时才会按它过滤。名字只能用中英文、数字、空格和 _ - . ，不超过 20 个字符。',
      categoryCustomEmpty: '（还没填）',
      categoryRequired: '请先填一个类型名，再选文件',
      categoryLoadFailed: '读取分类列表失败。可以选「其他…」自己填一个类型名。',
      processing: '处理中…',
      pick: '选择文件',
      pickDir: '导入整个目录',
      preview: '预览切片',
    ...
    bailian: {
      ...
      chunks: {
        ...
        category: '分类',
        // 和本地上传那边同一套「选其他 + 手填」，见 knowledgeCategory.js。
        // 文案没跟着复用是因为这个文件按页面/组件分组（见文件头），
        // 但「其他…」那个省略号不能省：它是不和真叫「其他」的分类撞上的唯一区分
        categoryOther: '其他…',
        categoryCustomLabel: '类型名',
        categoryCustomPlaceholder: '例如：护肤',
        categoryCustomHint: '新类型会成为一个独立分类。检索按关键词匹配，提问里出现这个类型名时才会按它过滤。名字只能用中英文、数字、空格和 _ - . ，不超过 20 个字符。',
      ...
      // 「没选」和「选了其他但没填名字」分开报：后者的用户去下拉框里找是找不到问题的
      needCustomName: '这几份选了「其他…」但还没填类型名：{names}。展开一份、把类型名填上再同步。',
